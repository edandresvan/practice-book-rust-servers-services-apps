...
/// Displays the sign-in for to the user.
///
/// # Arguments
///
/// * `template_engine` - Tera engine object to create the HTML page.
pub async fn show_signin_form(
  ...
  let contents: String =
    template_engine
      .render("signin.html", &context)
      .map_err(|error| {
        EzyTutorError::TeraError(format!("Tera template error: {error:#?}"))
      })?;

  Ok(HttpResponse::Ok().content_type("text/html").body(contents))
} // end fn show_signin_form()

/// Handle the user sign-in action.
///
/// # Arguments
///
/// * `template_engine` - Tera engine object to create the HTML page.
...
pub async fn handle_signin(
  ...
  let user_result =
    get_user_record_db(&app_state.db_pool, username.clone().as_str()).await;

  // If the user does not exist, return to the signin form with an error message.
  if user_result.is_err() {
    let mut context = tera::Context::new();
    context.insert(
      "errors",
      format!("Username '{username}' not found.").as_str(),
    );
    context.insert("current_username", &params.username);
    context.insert("current_password", &params.password);
    ...
    return Ok(HttpResponse::Ok().content_type("text/html").body(contents));
  }

  // Here, the user does exist, so check if the passwords match
  ...
  // If the passwords does not match, return to the signin form with an an error message
  if !does_password_match {
    let mut context = tera::Context::new();
    context.insert(
      "errors",
      format!("Invalid password for username '{username}'.").as_str(),
    );
    context.insert("current_username", &params.username);
    context.insert("current_password", &params.password);
  ...
  // And here, the user exists and provided a valid password.
  let mut context = tera::Context::new();
  context.insert("name", &params.username);
  context.insert("title", "Sign-in confirmation");
  context.insert(
    "message",
    format!("Hey {username}, you have successfully loged.").as_str(),
  );
